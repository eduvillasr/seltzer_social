...
export default function FollowingPage({ params: paramsPromise }: PageProps) {
  const params = use(paramsPromise);
  const [following, setFollowing] = useState<(User & { isFollowing?: boolean })[]>([]);
  ...
  async function load() {
    ...
    if (pUser) {
      const { data: followingData } = await getFollowing(pUser.id);
      const enriched = await Promise.all(
        (followingData || []).map(async (u: User) => {
          if (cuid && u.id !== cuid) {
  ...
  async function handleFollowToggle(userId: string, currentlyFollowing: boolean) {
    ...
    setFollowing(prev => prev.map(u => u.id === userId ? { ...u, isFollowing: !currentlyFollowing } : u));
  }
      ...
      <main className="max-w-md mx-auto px-4 pt-20 pb-32">
        ...
        {loading ? (
          ...
          <div className="glass-card text-center py-10">
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Not following anyone yet</p>
          </div>
        ) : (
